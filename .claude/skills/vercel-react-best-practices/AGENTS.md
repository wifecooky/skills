...
Comprehensive performance optimization guide for React and Next.js applications, designed for AI agents and LLMs. Contains 63 rules across 8 categories, prioritized by impact from critical (eliminating waterfalls, reducing bundle size) to incremental (advanced patterns). Each rule includes detailed explanations, real-world examples comparing incorrect vs. correct implementations, and specific impact metrics to guide automated refactoring and code generation.
...
3. [Server-Side Performance](#3-server-side-performance) — **HIGH**
   ...
   - 3.8 [Taint Sensitive Server Data](#38-taint-sensitive-server-data)
...
5. [Re-render Optimization](#5-re-render-optimization) — **MEDIUM**
   ...
   - 5.13 [Use useState Over useMemo for Correctness](#513-use-usestate-over-usememo-for-correctness)
6. [Rendering Performance](#6-rendering-performance) — **MEDIUM**
   ...
   - 6.10 [Use useId() for Instance-Safe DOM IDs](#610-use-useid-for-instance-safe-dom-ids)
   - 6.11 [Use Context Over cloneElement for Composition](#611-use-context-over-cloneelement-for-composition)
   - 6.12 [Use ownerDocument for Portal-Safe Event Listeners](#612-use-ownerdocument-for-portal-safe-event-listeners)
   - 6.13 [Use startTransition for View Transition Animations](#613-use-starttransition-for-view-transition-animations)
...
### 3.8 Taint Sensitive Server Data

**Impact: HIGH (prevents accidental exposure of secrets to client)**
...
### 5.13 Use useState Over useMemo for Correctness

**Impact: MEDIUM (prevents broken behavior when React discards memoized values)**
...
### 6.10 Use useId() for Instance-Safe DOM IDs

**Impact: MEDIUM (prevents ID collisions with multiple instances)**
...
function ThemeProvider({ children }: { children: ReactNode }) {
  return (
    <>
      ...
      {/* inline script targets hardcoded ID - collides with multiple instances */}
    </>
  )
}
```
...
### 6.11 Use Context Over cloneElement for Composition

**Impact: MEDIUM (works with Server Components, lazy, and portals)**
...
### 6.12 Use ownerDocument for Portal-Safe Event Listeners

**Impact: MEDIUM (ensures correct window context in portals and iframes)**
...
### 6.13 Use startTransition for View Transition Animations

**Impact: MEDIUM (enables smooth View Transition API animations)**
...
8. [https://shud.in/thoughts/build-bulletproof-react-components](https://shud.in/thoughts/build-bulletproof-react-components)
